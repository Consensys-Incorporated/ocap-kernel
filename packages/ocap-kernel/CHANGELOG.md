...
- A message from a remote peer is taken delivery of by the run loop, in a crank of its own, instead of in a savepoint nested inside whichever crank was open, which either deferred that crank's commit or committed a half-finished message ([#1103](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1103))
  - A message the kernel cannot deliver, or one for a remote that has gone, is logged and dropped instead of killing the run loop
  - A peer can have at most 200 messages waiting for their crank, and they take turns with the kernel's own work, so a peer that sends faster than cranks finish cannot hold that work up
