...
- Add `hasRemoteInfo` to the kernel store, which answers whether the kernel knows a remote at all without throwing for one it does not ([#1097](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1097))
...
- A notify, GC action or reap addressed to an endpoint that is not running is skipped rather than throwing out of the crank, which killed the run loop and, because the crank was rolled back, every boot after it. A vat's c-lists outlive it until cleanup reaches them, one vat per crank, and no remote exists at all until `initRemoteComms` runs — so a single `bringOutYourDead` from a peer was enough to stop a kernel ever booting again ([#1097](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1097))
  - The kernel's own half of a GC action is still performed for an endpoint it cannot tell, so the action is not re-derived forever; when that endpoint is a remote the kernel still has a record for, it is logged, because the peer keeps references the kernel has released
- Control-plane `terminateVat` is carried out by the run loop, as a run queue item. A vat's death is a set of store writes, and made from outside the run loop they landed in whichever crank happened to be open, for an unrelated rollback to undo after the caller had been told they succeeded ([#1098](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1098))
- `terminateSubcluster` retires a member the kernel has no handle for, and keeps going when one member will not die, instead of stranding the rest of the subcluster ([#1098](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1098))
