...
- A crank that aborts keeps the writes that follow the abort — the vat's death, the collection after it — instead of autocommitting them one statement at a time outside any transaction: the run loop takes a savepoint for the crank and one for the delivery, and only the delivery is rolled back ([#1089](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1089))
- A vat that exits gracefully stays dead when the rest of its crank then throws; before, the run loop's catch rolled the death record back and would have relaunched a vat whose worker was gone and whose callers had been answered ([#1089](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1089))
