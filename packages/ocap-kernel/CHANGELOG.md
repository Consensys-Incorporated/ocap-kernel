...
- A delivery to a remote peer is written down inside the crank making it and sent once that crank commits, instead of going out mid-crank. A crank that then failed took the kernel's record of the message back while the peer already had it, leaving the kernel's next send sequence behind what the peer had seen. The send sequence counters move with that record rather than in memory beside it, so a crank that rolls back takes them too ([#1101](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1101))
