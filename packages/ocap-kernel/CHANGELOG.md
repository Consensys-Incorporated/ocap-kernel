...
- The reference count audit credits the crank buffer, so a delivery that queued a message for later no longer reads as a leak and the audit runs before the flush hands that message on — a violation now kills the run loop before the answers the flush produces have gone out ([#1092](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1092))
