...
- `Kernel.stop` records its last-active timestamp best-effort, so a store that can no longer persist anything does not stop it from releasing remote comms, the vat workers and the database handle ([#1094](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1094))
- A crank whose savepoint release fails forgets its savepoints, matching what the store does with them. Before, the two disagreed from then on: every later crank released a `t0` the store no longer had, so the transaction was never ended and `Kernel.reset` could not restore the keys it had preserved ([#1094](https://github.com/Consensys-Incorporated/ocap-kernel/pull/1094))
